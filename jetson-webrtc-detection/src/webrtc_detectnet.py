...
class WebRTCDetectionServer:
    def __init__(self, input_uri, network="ssd-mobilenet-v2", threshold=0.5,
        ...
        print(f"[INIT] WebRTC検知サーバーを初期化中...")
    ...
    def print_detection_info(self, detections):
        ...
        """
        if len(detections) > 0:
            print(f"[DETECT] {len(detections)} 個の物体を検知:")
    ...
    def run(self):
        ...
        print(f"\n[START] WebRTC Object Detection Server が起動しました！")
        print(f"[URL] ブラウザでアクセス: http://{local_ip}:{self.port}")
        print(f"[URL] または: http://localhost:{self.port}")
        ...
        try:
            ...
            print("\n[EXIT] WebRTC Object Detection Server を停止します...")
            self.print_statistics()
            return True
